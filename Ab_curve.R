...
bf1 <- KIR.subset@meta.data %>% 
  filter(buffy_coat == 'BC351') %>%
  filter(!is.na(TRB_clone)) %>%
  mutate(groups = recode(HTO_classification, 'S1'='S1', 'S2'='S2', 'S3'='S3', 'S5'='S5'))

curve1 <- estimateAbundance(bf1, group="groups", ci=0.95, nboot=100, clone="TRB_clone", )

bf2 <- KIR.subset@meta.data %>% 
  filter(buffy_coat == 'BC357') %>%
  filter(!is.na(TRB_clone)) %>%
  mutate(groups = recode(HTO_classification, 'S6'='S6', 'S7'='S7', 'S8'='S8', 'S10'='S10'))
